...
**Maç karakteri, risk ve yorgunluk:** Atlas önde pres yapar; Solaris alçak bloktan topu kazandığında kısa süreli kontra koşuları ve dikine paslar arar; Coastal geniş çıkışları korur. Verdant'ın sabırlı planı topun iki yanında kısa pas açıları açar; dar hücum planı oyuncuyu zorunlu olarak çizgiye göndermez. Port Azure'ın dar ve alçak savunması, top merkezdeyken tehlikesiz kanat alıcısına açılıp ortayı boşaltmaz; top kanada gittiğinde veya koşucu ceza sahasına yaklaştığında takibe döner. Maçın son üçte birinde gerideki takım beklerini ve orta sahalarını ileri göndererek arkalarında alan bırakır; öndeki takım daha çok oyuncuyla topun gerisinde kalır. Bu tepki iki takımda ve iki hücum yönünde çalışır, rövanşta toplam skoru dikkate alır. Maç içinde verdiğin açık taktik talimatı kendi takımının otomatik skor tepkisinin önüne geçer.

`tests/match_variety_check.gd` aynı oyuncu özellikleri, başlangıç yerleşimi ve rastgelelik tohumu ile iki hücum yönünde gerçek koşuları, presin ulaşma süresini, kontra çıkışını ve skora bağlı risk değişimini karşılaştırır. `tests/team_identity_flow_check.gd` Atlas, Orman ve Liman'a karşı canlı maç akışında pas, kontrol ve duran topların devamını sınar.

Boşa çıkan ayakta müdahale, savunmacının ayağını yeniden basıp hızlanmasını geciktirir. Rakip uzanırken yapılan yön değişimi bu boşluğu kullanabilir; çalımın başarısı yine gerçek top teması ve boş çıkış alanına bağlıdır. Sprint ve normal koşu maç yorgunluğu biriktirir; dinlenme kısa süreli enerjiyi doldurur ama azalan enerji tavanını, sprint hızını ve ivmeyi tamamen geri getirmez. Kondisyon çubuğundaki kırmızı işaret o anki tavanı gösterir; devre arası da bu sınırı aşmaz. Oyuna giren yedek önceki oyuncunun maç yorgunluğunu devralmaz; kariyerde kendi maç öncesi kondisyonuyla başlar. Gol arayan rakip, yorgun kanadı için hızını ve kondisyonunu değerlendirerek yedek seçer. Doğrulama: `godot --headless --path . --fixed-fps 120 --script tests/match_dynamics_check.gd`.

**Bire bir atölyesi ve temel çalımlar:** Antrenman menüsündeki dördüncü seçenek sabit rakip, müdahale eden rakip ve serbest savunmacı aşamalarını açar. İki başarılı geçiş bir sonraki aşamaya götürür; `F3` / `LT + R3` aşamayı değiştirir, `R` / `R3` denemeyi yeniler. `2 + yön` / sağ analog yana: **yana çek**; `9` / sprint tuşu + sağ analog geri: **dur–kalk**; `0 + yön` / sprint tuşu + sağ analog ileri: **aç ve dolaş**. Son harekette top seçilen yana, oyuncu diğer yana gider; rakip fiziksel topu kazanabilir. Bu üç hareketin yön değiştiren dokunuşu ölçülen ayak temasında gerçekleşir; hazırlık ve çıkışta yakındaki top normal fizik kuvvetleriyle kontrol edilir. Hazırlıkta pasla vazgeçilebilir; temas başladıktan sonra hareketin bitmesi beklenir. Enerji, toparlanma ve top erişimi nedeniyle reddedilen girişler oyuncu kartının üstünde açıklanır. AI da aynı hareketleri kullanır; hareketlenen pas seçeneği varken topu saklayıp baskıyı çekebilir, en fazla 0,65 saniye sonra veya ikinci baskı geldiğinde yeniden karar verir. Doğrulama: `tests/ground_skills_check.gd` ve `tests/ai_hold_play_check.gd`.

Sprint basılıyken sağ analog yana hâlâ **yana çek**, hızlı ters yana giriş **elastico** yapar; topu istemeden açmaz. Dur–kalkta oyuncu topun yanında basıp yavaşlar, ikinci dokunuştan sonra top kontrolü koşuya bağlanır. Scoop topu kısa süre kaldırır; yatay yönlendirme yerçekimini iptal etmez. `tests/sprint_skills_check.gd` iki saha yönünde ve iki çıkış tarafında sprintten hareketi ve yeniden koşuyu doğrular. `tests/space_skill_balance_check.gd` zamanında yapılan hareketlerin koşarak gelen savunmacı karşısında çıkış sağladığını, geç girişlerin ve kapalı çıkışların top kaybıyla sonuçlanabildiğini sınar.
...
**Maç içi akıcılık:** Top gelmeden verilen normal pas, gerçek ilk kontrolden sonra kısa vuruş hazırlığı boyunca yakın kontrolü korur; pas, ses ve titreşim görünür ayak temasında birlikte çıkar. Islak zeminde yavaşlayan ara pası karşılayan oyuncu, topun artık ulaşamayacağı eski hedefe koşmak yerine güncel buluşma noktasına döner. Ara pasın kullanıcı tarafından seçilen yönü serbest kalır.

Kanattan son bölgeye girildiğinde uygun hücumcular ön direk, arka direk ve geriye çıkarılan pas için ayrı görevler alır. Hedefler kanattaki oyuncunun ilerlemesine ve boş alana göre değişir; görevler küçük konum değişikliklerinde oyuncular arasında sürekli el değiştirmez. Orta havadayken destek koşuları sürer; pası karşılayan veya kullanıcı tarafından kontrol edilen oyuncunun hareketi bu görevlerle değiştirilmez. Açık verkaç, bindirme ve savunma güvenliği görevleri korunur.

Top gelmeden çıkış yönünü hazırlamak ilk dokunuşu kısaltır ve toparlanmayı hızlandırır. Zayıf ayak ve deparla karşılama topu biraz daha açar; oyuncunun teknik ve denge özellikleri keskin dönüş hızına da etki eder. Teknik oyuncular yana çek hareketinden daha çabuk çıkar; top ve rakip çarpışmaları normal fizik kurallarını kullanır. Temiz ayakta müdahale topu taşıyıcının uzağına açar ve savunmacının daha erken peşinden gitmesine izin verir; boşa uzanma toparlanma süresine mal olur, top otomatik sahiplenilmez. Dönerek, geriye ve koşarken verilen pasların gövde dengesi ve destek ayağı süresi farklıdır. Gerçek temas, iki takım/iki devre/iki kanatta koşular ve hareket farkları: `tests/gameplay_polish_check.gd`.
...
Açılışta **Hızlı Maç / Enter / Xbox A** takım seçimini açar. Kendi kulübünü ve rakibini seç, formalarını belirle, ardından kadro ve taktik ekranından maça çık. **T** on iki seçenekli antrenman menüsünü açar. Takım seçiminin altındaki **DEVRE** düğmesiyle 2, 4, 6, 8, 10, 15 veya 20 dakika seçilir; iki devrenin toplamı ekranda gösterilir. Varsayılan 2 × 2 dakikadır. Gösterge her sürede 90 maç dakikasına ölçeklenir; duraklamalar ayrıca eklenir. Seçim sonraki hızlı maçlar için saklanır.
...
Ara pas ve şut nişanı kısa, saydam yön göstergesi ve ayrı güç çubuğuyla okunur; normal pas tek basışla ve güç çubuğu olmadan oynanır. Ara pas çubuğu 1,8 saniyede dolar; top gelmeden basılı tutulan ara pas da aynı hızı kullanır. İlk maç öğreticisi ve üstteki korner, taç, ofsayt, faul, kart ve diğer maç olayı yazıları gösterilmez. Ara pas yarışında AI topun ve rakibin hareketine göre, enerjisi izin verdiğinde sprint yapar. Ulaşılabilir havadan toplarda şut isteği kısa yaklaşma desteği alır; top erişim dışına saparsa boş vuruş iptal edilir ve hareket sürer. Kaleciler göğüs hizasındaki uygun topları iki elle karşılar. Ayakta müdahalede topun görünür ayağa yaklaşması gerekir; yalnızca oyuncunun yakınından geçmek topu uzaktan kapmaya yetmez.
...
Maç, seçilen süreye sahip iki yarıdan oluşur; başlayan maçın süresi sabit kalır. İlk yarının ve varsa uzatmasının sonunda oyuncular ve hakemler tünelden soyunma odasına gider; kamera saha çıkışını gösterir. Devre arası **Devam Et / Enter / kontrolcü onay tuşu** seçilene kadar bekler. Çıkış sürerken de devam edilebilir: takımlar doğrudan ikinci yarı santra düzenine yerleşir; sahaya giriş ve selamlama seremonisi tekrarlanmaz. Takımlar kaleleri değiştirir, seçtiğin rakip santrayla başlar. Devrede oyuncular mevcut kondisyon tavanına kadar 20 yüzde puan enerji kazanır; skor, kartlar, istatistikler ve oyuncu değişiklikleri korunur. Antrenmanda devre arası yoktur. Doğrulama: `godot --headless --path . --fixed-fps 120 --script tests/halftime_flow_check.gd`.
...
Kutlamadan sonra topun durduğu son konuma en yakın oyuncu aynı fiziksel topu alır; kendisi santrayı kullanmayacaksa merkezde bekleyen oyuncuya atar. Santrayı golü yiyen takım kullanır. İki takım kendi yarı sahasına döner; rakipler santra çemberinin dışında bekler. Top merkezde durup diziliş tamamlanınca düdük gelir. Santra Coastal Rovers'daysa **S'ye basıp bırakarak** başlat; rakipteyse rakip pasla başlar. Gol sonrası oyuncular ve top ışınlanmaz, maç saati kutlama/hazırlık sırasında ilerlemez. Antrenman doğrudan yeni şut denemesine döner.

## Oynanış derinliği: FIFA seviyesine doğru

**Temas kalitesi (şut ve pas hatası):** Her ayak ve kafa vuruşunun sapması gerçek temas anında belirlenir. Belirleyenler: ilgili teknik (içeriden bitiricilik, dışarıdan uzaktan şut, vole, kafa, orta, vizyon), yakın baskı ve soğukkanlılık, koşu yönüyle vuruş yönü arasındaki açı, sprint, zayıf ayak, gelişine vurulan topun hızı ve yüksekliği, omuz mücadelesi ve yorgunluk. Aynı temas her zaman aynı sonucu verir; zar atılmaz, top ayaktan çıktıktan sonra yönlendirilmez. Güç çubuğunda %72'nin üzeri isabetten hız alır: ortalama bir oyuncunun 25 metreden tam güçlü şutu çoğunlukla üst direğin üstüne gider, %80 güç çerçevede kalır. Çubuktaki çizgi bu sınırı gösterir; uzaktan aşırı güçte **AŞIRI GÜÇ · TOP HAVALANABİLİR** uyarısı çıkar. Plase vuruş daha isabetlidir. Baskısız yerden paslar tam isabetli kalır; baskı altında, vücudun arkasına, sprintte ve ilk dokunuşta verilen paslar sapar. Uzun ve havadan toplarda hafif ağırlık hatası vardır. Nişan oku niyet edilen vuruşu gösterir. Yapay zekâ aynı modeli kullanır; zorluk yalnızca karar kalitesini değiştirir. Doğrulama: `tests/strike_quality_check.gd`.

**Oyuncu kimliği:** Kayıtlı 13 çekirdek özelliğe ek olarak çeviklik, reaksiyon, soğukkanlılık, vizyon, orta, falso, uzaktan şut, şut gücü, vole, sıçrama, top kapma, araya girme ve degaj değerleri vardır. Bu değerler çekirdek değerlerden ve oyuncuya özgü sabit bir imzadan türetilir; kayıt göçü gerekmez, antrenman ilgili teknikleri de yükseltir. Her yeni değer gerçek bir formülü besler: dönüş hızı, ilk kontrol, AI'ın okuma süresi, ara pas ve orta hatası, falso miktarı, kafa sıçraması, ayakta müdahale erişimi ve pas arası uzanması. Hızlı maç kadrolarında güç ile denge artık ayrıdır. Her oyuncunun 1–5 **çalım yıldızı** vardır: elastico, rainbow ve McGeady için 4★; roulette, bacak arası ve topuktan topuğa için 3★ gerekir. Temel yer hareketleri ve aldatmalar herkese açıktır; AI da aynı sınıra uyar. En fazla dört **oyun stili** oyuncunun gerçek uzmanlığından gelir ve ilgili formülü küçük bir oranda iyileştirir: Plase Şut, Sert Şut, Aşırtma, İlk Dokunuş, Sürat, Çabuk Adım, Keskin Pas, Kavisli Orta, Baskıya Dayanıklı, Sezgi, Araya Girme, Hava Hakimi, Güçlü; kalecilerde Uzun Kol ve Ayak Oyunu.

**Zorluk ve oynanış ayarları:** Altı seviye vardır: Başlangıç, Amatör, Yarı Profesyonel, Profesyonel, Dünya Klası ve Efsane. Eski Kolay / Normal / Zor seviyeleri sırasıyla Başlangıç, Yarı Profesyonel ve Dünya Klası'na denk gelir. Aradaki seviyeler bu ayarlı tablolardan hesaplanır; Efsane Zor'un ötesine geçer. Eski kariyer kayıtları otomatik eşlenir. **Ayarlar → Oynanış** bölümünde FIFA tarzı sliderlar vardır; senin takımın ve CPU için ayrı ayrı ayarlanır ve varsayılan değer %50'dir: sprint hızı, ivme, şut hatası, pas hatası, şut hızı, kaleci yeteneği, ilk dokunuş hatası, savunma çizgisi yüksekliği ve genişliği, markaj sıkılığı, hücum koşu sıklığı ve sakatlık sıklığı. Sliderlar yalnızca var olan fiziksel ve taktik büyüklükleri ölçekler; ayarlar kaydedilir. Doğrulama: `tests/gameplay_sliders_check.gd`.

**Savunma zekâsı:** Koşucuyu kale tarafından takip etme ve ceza sahasına giren oyuncuyu karşılama artık iki takımda da çalışır. Senin takım arkadaşların da arkaya koşanı bırakmaz; kontrol ettiğin oyuncuya AI görevi verilmez. Topu taşıyan rakip baskısızsa ve kaleye dönükse savunma çizgisi birkaç metre geri çekilir. Markaj slider'ı koşucunun hangi derinlikte alınacağını belirler. Doğrulama: `tests/defensive_symmetry_check.gd`.

**Taktik:** Dokuz diziliş vardır: 4-4-2, 4-3-3, 3-5-2, 4-2-3-1, 4-1-2-1-2, 5-3-2, 3-4-3, 4-1-4-1 ve 4-4-1-1. Taktik ekranındaki diziliş satırı ◀ ▶ ile sayfalanır; sayfa değiştirmek dizilişi uygulamaz. **Oyuncu talimatları:** Kadro ekranında bir saha oyuncusunun kartı seçiliyken **T / Xbox Y** hücum talimatını (Geride kal · Dengeli · Hücuma katıl), **Y / R3** genişlik talimatını (İçe kat et · Dengeli · Geniş kal) değiştirir. Talimatlar gerçek destek koşularını ve savunma derinliğini değiştirir; hızlı maçta ayarlarla, kariyerde kulüp planıyla kaydedilir. Doğrulama: `tests/tactics_depth_check.gd`.

**Yeni hareketler:**
- **E + W / LT + RB, top sendeyken:** yan adımla sürme. Gövde aynı yöne bakar.
- **E + W / LT + RB, topsuzken:** kapatma (contain). Oyuncun kale tarafındaki boşluğa koşar ve mesafeyi korur.
- **W + Shift / RB + RT:** kontrollü sprint. %90 hızda, daha kısa dokunuşlarla sürersin.
- **V / R3 + yön, top gelmeden hemen önce:** ilk dokunuşta topu boşluğa itersin; top serbest kalır.
...
**Aynı bilgisayarda iki kişi:** Hızlı maç seçim ekranının üstündeki **OYUNCULAR** düğmesi üç seçenek sunar: Tek kişi, 2 kişi · karşılıklı ve 2 kişi · aynı takım. İkinci kişi bir kontrolcü kullanır; iki kol varsa klavye gerekmez. Her kişinin seçili oyuncusu, şut ve pas şarjı, nişanı, sprinti ve oyuncu değiştirme mantığı ayrıdır. Klavyede basılı tutulan tuşlar yalnızca klavyedeki kişiye aittir. P2'nin halkası mavidir; oyuncusunun üstünde **P2** etiketi ve güç çubuğu görünür. Karşılıklı maçta P2 kendi takımının duran toplarını ve santrasını da kullanır. Aynı takımda iki kişi aynı oyuncuyu seçemez. Kadro ekranı ve anlık taktik kısayolları P1'e aittir; kariyer her zaman tek kişiliktir. Doğrulama: `tests/local_multiplayer_check.gd`.

## Hızlı maç, kadro ve ayarlar

Hızlı Maç sırası: **Takım ve rakip seçimi → İlk 11 ve taktik → Seremoni → Maç**. Sekiz kurgusal kulübün ayrı isimleri, kadroları, armaları ve iki forma seçeneği bulunur: Coastal Rovers, Atlas FC, Ironvale FC, Solaris United, Verdant Rovers, Northstar FC, Port Azure Athletic ve Redmesa SC. Formayı 3D oyuncu üzerinde görürsün; isimler, formalar, yayın grafikleri ve stadyum tabelaları seçime göre güncellenir.
...
- **Taktik:** Dokuz diziliş (4-4-2, 4-3-3, 3-5-2, 4-2-3-1, 4-1-2-1-2, 5-3-2, 3-4-3, 4-1-4-1, 4-4-1-1); savunmacı/dengeli/hücumcu anlayış, pres yoğunluğu, savunma çizgisi ve oyuncu talimatları. Altı zorluk seviyesi rakibin karar süresini, okuma keskinliğini ve baskısını değiştirir.
...
Ana menüde hızlı maç, kariyer ve antrenman aynı sütunda bulunur. Kayıt yoksa **Yeni Kariyer** boş bir yuvada kulüp seçimini açar. Kayıt varsa kulüp arması, adı ve tarihle birlikte **Kariyere Devam Et** görünür; açık oturuma veya en son kaydedilmiş geçerli kariyere doğrudan döner. **Yeni / Kayıtlar** düğmesi üç kayıt yuvasına erişim sağlar. Fare, klavye ve kontrolcü aynı görsel sırayı izler. Doğrulama: `godot --headless --path . --fixed-fps 120 --script res://tests/home_menu_check.gd`.

Kulüp seçiminde **Devre Süresi** ve **Rakip Zorluğu (Kolay / Normal / Zor)** seçilir. Bu tercihler her kariyerin kendi kaydına yazılır ve maçlarında yeniden uygulanır; hızlı maç tercihlerini değiştirmez. Eski kayıtlar 2 dakikalık devreler ve Normal zorlukla açılır. SEFC Premier Lig ve SEFC Birinci Lig, farklı güçlere ve formalara sahip toplam **36 kurgusal kulüp** içerir. Her ligde 18 takım, çift devreli 34 haftalık sezon vardır. İlk üç alt lig takımı yükselir, üst ligin son üçü düşer; sezon ödülleri, yeni fikstür ve sezon arşivi bir sonraki yıla taşınır.
...
- **SEFC Lig Kupası:** İki ligden 36 kulüp. Sekiz takım ön eleme oynar, 28 takım sonraki tura doğrudan geçer. Son 32'den finale kadar tek maçlı eleme; şampiyona €1,8 milyon.
...
- **Süper Kupa:** Lig şampiyonu ile SEFC Lig Kupası şampiyonu karşılaşır. Aynı kulüp ikisini kazanırsa lig ikincisi katılır; kazanana €650 bin. İlk kariyer sezonunda önceki sezon bulunmadığı için Avrupa katılımı ve Süper Kupa, başlangıç kulüp güç sıralamasından belirlenir.
...
İspanya, Portekiz, İtalya, Almanya, Fransa, İngiltere ve Hollanda'nın her birinde sekiz kurgusal kulüp ve çift devreli 14 maçlık sezon vardır. Ayrı **Türkiye Süper Ligi** ile toplam **10 lig, 110 kulüp, 2.640 başlangıç oyuncusu** bulunur. Bu liglerden birinde kariyere başlayabilir veya teknik direktör merkezinden kulüp değiştirebilirsin. İspanya, Portekiz, İtalya ve Almanya’dan ilk iki; Fransa, İngiltere, Hollanda ve Türkiye’den şampiyon sonraki Şampiyonlar Kupası’na katılır. SEFC Premier Lig’in dört temsilcisiyle birlikte turnuva 16 takım olarak kalır. Yerel iki ligdeki 18 takım / 34 hafta ve yükselme-düşme sistemi korunur; yabancı liglerde henüz ikinci kademe yoktur. Oyuncular 13 farklı milliyetten gelir. Transfer ekranında ülke seçebilirsin. Ulusal liglerde **Yabancı** filtresi o ligin ülkesine göre çalışır; bağımsız SEFC liglerinde yerli/yabancı ayrımı yapılmaz. Milliyet transfer, kiralama ve kayıt yüklemede korunur; özellik bonusu veya yabancı kotası uygulamaz.
...
Kazanılan kupa finali sonrasında **kaptanın kupayı teslim aldığı, başının üstüne kaldırdığı ve takımın madalya ile kutladığı bir tören** vardır. Kupa yükseldiğinde kulüp renklerinde konfeti ve tribün tepkisi başlar; kamera önce kaptana yaklaşır, ardından takım kutlamasına açılır. Sahadaki kaptan seçilir; kaptan yoksa uygun bir takım arkadaşı kupayı kaldırır. Lig sezonunun son kullanıcı maçında kesinleşmiş şampiyonluk için de tören açılır. Kariyer ekranından simüle edilerek kazanılan finalde aynı tören oynatılır. Bu sezon kazandığın lig veya kupayı **Lig & Kupalar → Töreni izle** ile kayıt yükledikten sonra da tekrar izleyebilirsin. Space / Enter veya koldaki onay tuşuyla geçilir, Esc / Start ile duraklatılır; simülasyon ve tekrar izleme sonunda kariyer merkezine dönülür. Tören sonuçları, kondisyonu ve bütçeyi değiştirmez; şampiyonluk ödülü yalnızca bir defa yazılır.

Tören doğrulaması: `godot --headless --path . --fixed-fps 120 --script tests/trophy_presentation_check.gd`. Kaptan/kaleci/yedek lider seçimi, deplasman ve rakip galibiyeti, ellerin kupa kulplarına teması, kaldırma-konfeti sırası, duraklatma, kol/klavye ile geçiş, simülasyon, kayıt yükleme ve tekrar izleme kontrol edilir. Grafik testi için `--headless` kaldırılıp `-- --visual` eklenir; görüntüler `/tmp/sefc-trophy-*.png` yoluna yazılır.

### Hareket, temas ve sunum cilası

Ani dönüşlerde destek bacağına ağırlık aktarımı ve gövde dönüşü, vuruş sonrasında omuzların ve dizin toparlanması, devam eden omuz mücadelesinde temas tepkisi belirginleştirildi. Bu görsel katmanlar hareket girdisine bekleme eklemez. Hafif pas, sert şut, direk ve kaleci eldiveni farklı ses/titreşim profilleri kullanır; vuruş hazırlığı ses üretmez, geri bildirim gerçek temas olayında verilir. Kaleci yakalama, çelme ve penaltı serisi kurtarışları eldiven sesini kullanır.

Gol tekrarı gerçek vuruş karesini kaydeder. İlk açı vuran oyuncuyla topu, ikinci açı kaleciyle topu birlikte izler; vuruş ve gol çizgisi anlarının üzerine kamera geçişi gelmez. Kamera aynı açı içinde yakınlaştırıp uzaklaştırmaz. Kupa başlığı ilk gösterimden sonra sol üstte küçülür, para ödülü sağ üstte kalır. Törene yedekler ve teknik direktör katılır; kaptan kupayı takım arkadaşına verir, oyuncular omuz omuza takım fotoğrafına geçer. Tören oyuncuları maç kadrosuna veya kariyer istatistiklerine eklenmez.

**Ayarlar → Görüntü → Maç sunumu** seçeneği **Tam / Kısa** olarak kaydedilir. Tam sunum mevcut giriş, gol sevinci, değişiklik ve top toplama akışlarını korur. Kısa sunumda giriş yaklaşık 2,4 saniyeden, gol sevinci ve duran top hazırlığı yaklaşık 2,2 saniyeden, değişiklik yaklaşık 2,5 saniyeden sonra kısa bir kararmayla tamamlanır. Kart ve ihraç gibi zorunlu işlemler beklenir. Tekrar vuruştan 0,6 saniye öncesini korur; normal tekrar hızı 0,95×, gol çizgisindeki yavaş çekim 0,48× olur. Kupa töreni 23 yerine 11,5 saniyede aynı aşamalardan geçer. Tercih maç saatini, oyun hızını, kondisyonu ve değişiklik hakkını değiştirmez.

**Transfer pazarı / Görüşme → Kadronla karşılaştır** ekranında aday ile mevcut oyuncunun gerçek hız, ivme, teknik, pas, şut, dayanıklılık ve kondisyon değerleri yan yana görünür; kalecilerde uygun kaleci özellikleri gösterilir. Aynı mevkideki ilk 11 oyuncusu varsayılan olarak seçilir, başka bir oyuncu da seçilebilir. Yeni satın alınan veya kiralanan oyuncu ilk oynadığı maçta kısa bir tanıtım kartı alır. İlk maç durumu kayda yazılır; yalnızca tanıtımı görmek görünüm sayısını artırmaz, tamamlanan maç veya simüle edilen ilk katılım tanıtımı tüketir.

Doğrulama: `tests/polish_flow_check.gd` temas zamanlaması, ayar kaydı, kısa sunum, değişiklik, taç/köşe vuruşu, transfer karşılaştırması, ilk maç kaydı, kupa devri ve tören temizliğini sınar. `tests/polish_replay_check.gd` iki hücum yönünde ve iki sunum tercihinde vuruş/kaleci kadrajını, geçişleri ve canlı sahnenin geri yüklenmesini sınar. Her ikisi `godot --headless --path . --fixed-fps 120 --script tests/<dosya>.gd` ile çalışır; grafik çalıştırmada `--headless` kaldırılıp `-- --visual` eklenir, görüntüler `/tmp/sefc-*.png` altında tutulur. Ses ve titreşim profilleri otomatik olarak doğrulandı; fiziksel kontrolcüde titreşim hissi için kullanıcı denemesi gerekir.
...
| S tek basış | Yönündeki uygun arkadaşına otomatik hızla pas; top yaklaşırken erken pas komutu. Diğer topsuz durumlarda pas iste |
...
**LB + Y — havadan uzun pas:** Top sendeyken LB'yi tut, sol analogla yönü belirle ve Y'yi basılı tutup bırak. Güç 0,65 saniyede dolar; kısa basış yakın, uzun basış uzak mesafeye gider. Ok, takım arkadaşına kilitlenmeden seçtiğin yönü izler; saha sınırına yönelsen bile içeri çevrilmez. Yön ve güç göstergesi vuruştan önce görünür. LB'yi önce bırakmak pası bozmaz. Tek başına Y serbest nişanlı ara pas / savunmada kaleci çağırma olarak kalır.
...
Coastal Rovers beyaz-yeşil formayla ilk yarıda **yukarıdaki**, ikinci yarıda **aşağıdaki kaleye** hücum eder. Takım kontrolü varsayılan olarak açıktır: S/A/Y pasından sonra alıcıya, topu kazanan takım arkadaşına ve savunmada belirgin biçimde daha uygun oyuncuya geçilir. LB/Q ile yaptığın seçim kısa süre korunur; kontrol işareti yakın oyuncular arasında sürekli atlamaz. Gelen pasta analog/yön girişi yoksa alıcı topu karşılar; yön verdiğinde hareket kontrolü tamamen sendedir. Tab ile tek oyuncuda kalma açıldığında pas sonrası kontrol değiştirilmez; pas verip boşa koşarak yeniden isteyebilirsin. Topsuzken S/A oyuncu değiştirmez: el kaldırıp pas istersin. Takım arkadaşın yaklaşık üç saniye içinde uygun pas yolunu arar; koşuna göre topu önüne bırakır, gerekirse havadan oynar. Markajdaysan boşa çıkmalısın. Rakip pası kesebilir; gelen top fiziksel olarak kontrol edilir.
...
Top rakipteyken veya boşta yavaş ilerlerken otomatik seçim, topa yakın ve müdahale edebilecek oyuncuyu önceler; 8 metre uzaklaşma / 5 metre avantaj şartı yoktur. Yakınlıkla birlikte topun kısa süre sonraki konumu, oyuncunun hareket yönü ve kondisyonu değerlendirilir. Topun 2,8 metre yakınında mücadele edebilen seçili savunmacı korunur; rakip veya pas oyuncuyu geçip uzaklaşıyorsa uygun arkadaşına geçilebilir. Rakibin top sürerken dokunuşlar arasında topu ayağından ayırması uzun bir pas sayılmaz. Rakibin yerden paslarında tahmin sonraki 0,72 saniyeyle sınırlıdır; birkaç saniye sonraki olası buluşma kontrolü uzaktaki savunmacıya taşımaz. Küçük mesafe farkları seçim değiştirmez; belirgin bir müdahale fırsatında beklenmez. Yerde kalan veya ihraç edilen oyuncu atlanır, yakın mesafede başlamış müdahale tamamlanır. **LB / L1 / Q** aynı yakın oyuncu sıralamasını kullanır; sol analog ve yön tuşları bu seçimi saptırmaz. Belirli bir oyuncuya yönlü geçiş **sağ analogla** yapılır. Manuel seçim bir saniye korunur; tahmin bu süreyi kesmez, gerçek top teması her zaman önceliklidir. `tests/defensive_selection_check.gd`, iki yarıda canlı top sürüşü, 30/60/120 Hz'de dokunuşlar arası kararlılık, yakın savunma, pas arası, klavye ve Xbox/PlayStation geçişlerini doğrular.
...
Normal yerden pas için yön verip **S / Xbox A / PlayStation ×** tuşuna bir kez bas. Güç çubuğu açılmaz; yönündeki uygun arkadaş seçilir, hız mesafeye göre hesaplanır. Yön girişi yoksa oyuncunun baktığı taraf kullanılır. Aynı yöndeki seçeneklerde yakın oyuncu önceliklidir; o yönde kimse yoksa top boşluğa gider. Kapalı pas yolu başka taraftaki oyuncuya otomatik yönlendirilmez. Ayarlar → Görüntü & Oyun → Pas yardımı ile Manuel / Yarı yardımlı / Yardımlı seçilebilir; Manuel seçimi hedef yardımını kapatır. **Y / Xbox Y / PlayStation △** ara pası basılı tutup bırakılır: yön tamamen oyuncudadır; ok takım arkadaşına veya koşucunun önüne otomatik çevrilmez. Güç, seçilen doğrultudaki mesafeyi belirler; çubuk 1,8 saniyede dolar. Bu serbest nişan bütün pas yardımı ayarlarında geçerlidir. Ara pası hazırlarken yön girişi hemen uygulanır, analog bırakıldığında seçilen yön korunur. Takım arkadaşları gönderilen topa koşabilir. Şut, ara pas, havadan pas ve orta güç göstergelerini korur. Top çıktıktan sonra alıcıyı takip etmez.

Sana gelen top için normal pas veya ara pas komutu **0,30 saniye** saklanır. Gerçek karşılama gerçekleşince normal pas başlar; ara pas hâlâ tutuluyorsa güç hazırlığı sürer, bırakılmışsa vuruş hazırlanır. Komut oyuncuya bağlıdır; süre dolması, rakip teması, oyuncu seçimi, darbe, mola, düdük, yeni maç ve kontrolcü bağlantısının kesilmesiyle temizlenir. Topun gelmediği topsuz durumda pas isteme korunur. Kontroller: `tests/pass_skill_check.gd`, `tests/team_control_check.gd`, `tests/attacking_combos_check.gd`.

Ara pas, havadan pas, şut, orta ve duran toplar ortak bir kısa ok ve parçalı güç göstergesi kullanır. Ok zemine oturur, güçle dolar ve kameraya göre okunur bir boyutta kalır; yerden pas mint, şut altın, havadan vuruşlar açık mavidir. Uzun uçuş çizgisi, zemin izdüşümü ve vuruş öncesi kesin hedef halkası gösterilmez. Alt ortadaki küçük kart vuruş türünü, gücünü ve varsa kapalı pas yolu/yüksek şut uyarısını taşır. Gösterge hazırlıkta görünür; normal vuruşta tuş bırakılınca, erken verilmiş kafa/vole komutunda ise gerçek temas gerçekleşince kaybolur. Top havadayken gerçek uçuşun düşüş işareti korunur. Bu gösterimler top fiziğini, nişan yardımını veya vuruş zamanını değiştirmez. Görsel kontrol: `godot --path . --script tests/aim_indicator_preview.gd`.

Top kontrolündeyken klavyede şut gücü doldurulurken sol/sağ tuşlarına kısa dokunuşlarla nişan ince ayarlanır. D bırakılınca top gösterilen yönde çıkar. Fareyle şut nişanı da yumuşak döner. Xbox'ta X basılıyken sol analog şut yönünü seçer; başlangıç yönünün çevresindeki 14 derece sınırı kontrolcü için uygulanmaz. Sol analogda tam eğimle nişan dönüşü yaklaşık %38 yavaşlatılmıştır; hafif eğim daha küçük düzeltmeler yapar. Frikik, korner, taç ve penaltı hedeflemesi aynı hassas analog eğrisini kullanır. Normal pas doğrudan seçilen yöne oynanır; ara ve havadan pas hazırlığında yön girişi hemen uygulanır. Analogu merkeze bırakınca hedef hemen sabit kalır; hareket hassasiyeti ayarı nişanı hızlandırmaz. Oyuncunun koşu tepkisi korunur. **D-pad veya sağ analog**, koşu yönünden bağımsız nişan verir: sol analogla sola koşarken sağa şut çekebilirsin. Bu ayrı nişanı bırakmak seçilen yönü korur; koşu yönüne geri atlamaz. X bırakıldığında top ekrandaki okun yönünde çıkar. D-pad duran toplarda da sağ/sol nişan düzeltmesi yapar. Xbox/PlayStation, 30/60/120 Hz, duran toplar ve penaltı serisi kontrolü: `godot --headless --path . --fixed-fps 120 --script tests/aim_precision_check.gd`.

Kafa, vole ve yarım volede **D / Xbox X / PlayStation □** ile vuruş iste, yön tuşları veya sol analogla çıkış yönünü doğrudan seç. Dört yön, çaprazlar ve kontrolcüde tüm analog açılar kamera yönüne göre uygulanır. Şut tuşunu erken bırakırsan güç sabitlenir; topa temas edene kadar yönü değiştirebilirsin. Yönü bırakmak son seçimi korur. Bekleyen vuruşun oku ve güç göstergesi temasa kadar görünür. Sağ analog veya D-pad ayrı nişan olarak kullanılabilir; bırakıldığında koşu yönüne geri atlamaz. Yerden gelen topa gelişine şutta da yön seçimi anındadır. Kafa/ayak teması olmadan topa vurulmaz; çıktıktan sonra topun yönü değişmez. Klavye, Xbox/PlayStation, kamera ve devre yönü, gerçek kafa/vole/yarım vole teması: `tests/first_time_aim_check.gd`.
...
Kaleciler top-kale açısına yerleşir, ceza sahasında ulaşabilecekleri boş topa veya yalnız kalan hücumcuya çıkar. Şuta tepki vermeleri zaman alır; yön ve yükseklik tahminleri kusursuz değildir. Yere vurulan kafa toplarında sekme ve zemin direnci hesaba katılır; yerden yükselen topa yanlışlıkla alçak ayak kurtarışı hazırlanmaz. Sert köşe şutları ve yakın mesafeli bitirişler kaleciyi geçebilir; kurtarış için topun eldivene veya gövdeye gerçekten yaklaşması gerekir. Yorgunluk, yağmur ve görüşü kapatan oyuncular hata olasılığını artırır. Ortanın düşüşünü hesaplayıp erişilebilir topa fiziksel olarak sıçrarlar. Eldivene gelen tutulabilir hızdaki rakip topunu ayakta, yere eğilerek veya uzanarak kontrol edebilirler. Tutuş temasında gelen hız sönümlenir; top konumu değiştirilmeden ellerle taşınır ve eski şut hızıyla eldivenden fırlamaz. Sert topları genellikle güvenli yana çelerken bazen önlerine sektirip ikinci vuruş fırsatı bırakırlar. Rakip kalecinin tepki ve tahmin becerisi zorluk ayarına bağlıdır. Takım arkadaşının geri pası elle yakalanmaz. Sekme tahmini, iki tarafta kuru/yağmurlu kafa topları, tutuş ve dağıtım kontrolü: `tests/keeper_handling_check.gd`.
...
Yere düşen top her sekmede ileri hız ve sekme yüksekliği kaybeder. Vuruştan kalan fazla dönüş, sonraki zemin temasında topu yeniden hızlandıramaz; yeni bir oyuncu vuruşu veya dokunuşu normal gücünü korur. Vuruş önizlemesi de aynı sekme kaybını kullanır. Kuru/ıslak zeminde düz ve falsolu uçuşlar, dik düşüş, 30/60/120 Hz ve zemin altına taşma kontrolü: `godot --headless --path . --fixed-fps 120 --script tests/ball_bounce_check.gd`.
...
Antrenman menüsünde **12 seçenek** bulunur. Dört serbest atölye: **Serbest antrenman** (11 oyuncu), **Orta & kafa**, **Serbest vuruş** ve **Bire bir atölyesi**. Sekiz puanlı çalışma: **Koni parkuru, Hedef pası, Şut isabeti, Orta açma, İlk kontrol, Penaltı serisi, Top kazanma ve Kaleci dağıtımı**. Koniler, pas kapıları, orta bölgeleri ve kale hedefleri sahada görünür; sonuçlar gerçek top hareketi, vuruş, kontrol ve müdahaleden hesaplanır. İlk kontrol çalışmasında başlangıçtan farklı altı karşılama alanına koşulur; gerçek pası aldıktan sonra top sürerek konili çıkış kapısından geçilir. Pas oyuncunun beklediği yere yönlenmez; yerinde durmak veya yalnızca topu karşılamak puan kazandırmaz. Her puanlı çalışma altı deneme içerir; 600 üzerinden toplam, 100 üzerinden başarı puanı ve A–F notu gösterilir. **Ana menüden yapılan pratik hiçbir kariyer oyuncusunu geliştirmez.**

**Kariyer merkezi → Haftalık antrenman** veya **Gelişim planları → Haftalık antrenman** üzerinden yedi günlük beş çalışma yönetilir. Antrenör sağlıklı, 30 yaş altı ve potansiyeline ulaşmamış A takım/akademi oyuncularını önerir; oyuncuyu ve çalışmayı değiştirebilir, her çalışmayı kendin oynayabilir veya simüle edebilirsin. Kalecilere dağıtım çalışması verilir. Otomatik açıkken takvimin sonraki gününde kalan çalışmalar tamamlanır; yedi gün sonra yeni liste açılır. Her oyuncu haftada bir çalışma yapar; tamamlanan satır değiştirilerek tekrar gelişim kazanılamaz. Sonuç ekranından sonraki çalışmaya doğrudan geçilebilir.

Yüksek başarı puanı daha çok gelişim puanı verir; yaş ve antrenman yükü bu miktarı etkiler. 100 birikmiş gelişim puanı, çalışmanın hedeflediği bir özelliği bir kademe artırır; potansiyel sınırı ve mevcut uzun vadeli gelişim planı korunur. Oyuncunun aynı çalışmada **kariyer içinde oynayarak** kazandığı en iyi not sonraki otomatik çalışmalarda kullanılır. Sıfır başarı puanı ve yarıda bırakılan çalışmalar gelişim sağlamaz. Haftalık program, notlar ve gelişim kariyer kaydında saklanır; eski kariyerlere de program eklenir.

**R** puanlı çalışmada mevcut denemeyi sıfır puanla geçer; serbest atölyelerde yeni deneme başlatır. Kontrolcüyle mola menüsündeki yeni deneme seçeneği kullanılır; bire bir atölyesinde **R3** kısayolu da vardır. **T / View / dokunmatik yüzey** serbest pratikte çalışma seçimine, kariyer çalışmasında haftalık programa döner. Menü fare, klavye, Xbox ve PlayStation ile kullanılabilir. Serbest orta atölyesinde pas tuşuyla erken orta istenir, şut tuşuyla kafa vurulur; serbest vuruşta noktayı seçip **A** ile barajlı vuruşa geçilir.

Doğrulama: `tests/training_progression_check.gd` haftalık sınır, hedef özellik, potansiyel, otomatik not ve kayıt uyumluluğunu; `tests/training_sessions_check.gd` gerçek top fiziği, kariyer oyuncusuyla oynama, menü geçişleri ve ana menü/kariyer ayrımını; `tests/training_playability_check.gd` gerçek yön ve aksiyon girişleriyle koni parkuru, savunma ve altı penaltıyı sınar. `tests/training_modes_check.gd` mevcut serbest atölyeler ile kontrolcü kullanımını korur. Çalıştırma: `godot --headless --path . --fixed-fps 120 --script res://tests/training_sessions_check.gd`.
...
Top yeni oyuncu ölçeğine göre küçültülmüştür; model, çarpışma, yere yerleştirme, pas/şut tahminleri ve kale çizgisi kontrolleri aynı yarıçapı kullanır. İki takımın depar hızı ve deparın hızlanma ivmesi %20 artırılmıştır. Tam enerjili, hız özelliği 72 olan oyuncunun deparı yaklaşık 8,49 m/s'dir; hız özelliği ve yorgunluk bu değeri etkiler. Normal koşu, yorgun yürüyüş ve saniyelik kondisyon tüketimi mevcut ayarlarını korur.
...
Hızlı maçta kulüp armaları, canlı forma modelleri ve üç güç göstergesi; taktiklerde sahaya yerleşmiş oyuncu portreleri kullanılır. Antrenmandaki on iki kart, çalışmanın hedefini anlatan saha çizimleriyle dört sıra hâlinde gösterilir. Kartlarda kısa açıklama ve puanlı çalışma/serbest atölye ayrımı bulunur. Kariyer, antrenman ve mola menüsü ortak koyu yüzeyler, açık yeşil vurgu ve belirgin kol odağı kullanır.
...
### Okunabilirlik ve oyun testleri

**Ayarlar → Erişilebilirlik** bölümünde maç skorunu, oyuncu bilgi kartını ve kontrol rehberini %100 / %115 / %130 boyutunda kullanabilirsin. Rehberde açıklamalar satıra bölünür; uzun listeler fare, klavye veya kontrolcü ile kaydırılır. İsteğe bağlı takım şekilleri sahada ve mini haritada takımı daire, rakibi kare ile gösterir; kontrollü oyuncu dolu okla, top mini haritada baklava şekliyle ayrılır.

Kontrol rehberi F1 veya menüden açılır. Maç içinde otomatik ilk maç öğreticisi bulunmaz.

Müdahale toparlanırken tekrar basıldığında kalan süre gösterilir; bu basış sonraya saklanıp beklenmedik bir hamle üretmez. Animasyon geçişleri, top/zemin yerleştirmeleri dahil ekranda görülen son pozdan başlar. Omuz temasının yönü değiştiğinde iki oyuncunun karşılıklı yüklenmesi yumuşar, ayrılınca çözülür.

Yerel maç raporları açıkken yeni gerçek maçlar `user://playtests/` klasörüne JSON olarak yazılır. Şut konumu/açısı/türü, gerçek kurtarış veya gol, yağmur/çamur, enerji/yorgunluk, ilk kontrol hataları, oyuncu seçimleri, pas bekletme ve müdahale olayları kaydedilir. Son düdükte **Maçı değerlendir** düğmesiyle kontrol, denge ve animasyona 1–5 puan ve isteğe bağlı yorum eklenebilir. Dosyalar cihazda kalır; ağ üzerinden gönderilmez. Klasör aynı ayar sayfasından açılır. Olay, şut ve örnek sınırına ulaşılırsa raporda `truncated` işaretlenir. Başsız testler ancak açıkça istendiğinde rapor üretir ve `automated` olarak etiketlenir.

Birden fazla raporu özetlemek için:

```sh
python3 tools/summarize_playtests.py /rapor/klasoru --output /tmp/oyun-testleri.md
```

Özet varsayılan olarak yalnızca insan maçlarını kullanır; otomatik testler `--include-automated` ile ayrı etiketli gruplarda eklenebilir. Sürüm, zorluk, yağmur, yorgunluk, şut açısı, mesafe ve türü ayrı tutulur. Kaynağı belirsiz veya tamamlanmamış şutlar gol/kurtarış diye uydurulmaz. Az denemeli gruplar tek başına denge kararı vermek için yeterli değildir.

İnsan denemelerinde farklı oyuncuların kuru ve yağmurlu maçlar oynaması, aynı gol yöntemini farklı rakiplere karşı denemesi ve yorulmuş kanat oyuncusunu taze bir yedekle karşılaştırması gerekir. Maç sonu yorumuna pozisyonu ve beklenen hareketi yazmak, puanı kayıtla ilişkilendirmeyi sağlar. Bu geliştirme sırasında insan katılımcılı maç yapılmadı.

Doğrulama: `tests/match_experience_check.gd` rehber, kayıt, erişilebilirlik, baskı altında yönlü ilk dokunuş ve temas geçişlerini sınar. `tests/experience_balance_check.gd` iki hava durumu, iki kaleci enerji seviyesi, üç geliş açısı ve iki köşeyi kapsayan 72 kontrollü şut üretir; sonuç `/tmp/football-experience-balance.json` dosyasındadır. Bunlar tam maç kazanma oranı veya gol açığı bulunmadığı iddiası değildir. Mevcut pas, oyuncu seçimi, temas, kaleci ve yağmur testleri ayrıca korunur.

Maç süresi, iki yarının bitişini, ilave süreyi, uzatma saatini ve kalıcı kondisyon yükünü birlikte ölçekler. Normal koşunun bir devrelik enerji maliyeti korunur; kısa sprintin saniye başına enerji tüketimi değişmez. Süre, kayıt geçişleri ve kontrolcü seçeneklerinin doğrulaması: `godot --headless --path . --fixed-fps 120 --script tests/match_duration_check.gd`.

### Oyuncu kalitesi ve nadir yıldızlar

Oyuncu güçleri kariyer, hızlı maç ve altyapı için yeniden dengelendi. Genel seviye düşük kalırken liglerin güçlü kulüpleri de belirgin kadrolara sahip: başlangıç dünyasındaki 2.640 oyuncunun ortalaması **66,21**; **285 oyuncu 80+**, **111 oyuncu 85+**, yalnızca **9 oyuncu 90+**. Mevki ağırlıkları ve oyuncunun güçlü/zayıf özellikleri korunur. Yüksek potansiyel de nadirdir; birikmiş antrenman puanı potansiyelin üstüne gelişim sağlayamaz.

İsimler kurgusal kalır; liglerdeki favori sayısı ve kadro farkları gerçek futbolu çağrıştırır. Bu dağılım [UEFA kulüp sıralamalarının](https://www.uefa.com/nationalassociations/uefarankings/) genel hiyerarşisinden esinlenen bir oyun dengesi tercihidir; kurgu kulüplerin resmî derecelendirmesi veya gerçek kulüplerle birebir eşleştirmesi değildir. Sayılar başlangıç ilk 11'inin yuvarlanmış ortalamasıdır; kadro değişikliğiyle değişir.

| Lig | Başlangıç güç dağılımı |
| --- | --- |
| İspanya | Madrid Aurora **85**, Barcelona Marina **84**; Bilbao FC **76**, Valencia United **73**; diğerleri **67–70**. |
| İngiltere | London Borough **84**, Bristol Sport ve York United **83**, Oxford FC **81**; Leeds Athletic **78**, Derby United **74**; alt grup **70–71**. |
| İtalya | Milano Stella **83**, Napoli Sport **82**, Roma Aurelia ve Torino United **81**; orta sıra **72–75**, Verona Sport **68**. |
| Almanya | München Wald **85**; Hamburg United **81**, Berlin Adler **80**; diğerleri **70–75**. |
| Fransa | Paris Lumière **85**; Lyon Rivière **78**, Lille United **76**; diğerleri **68–73**. |
| Portekiz | Lisboa Atlântico ve Porto Estrela **81**, Braga United **79**; diğerleri **67–70**. |
| Hollanda | Amsterdam Haven ve Utrecht United **80**, Delft Sport **79**; diğerleri **65–71**. |
| Türkiye | Boğaziçi Yıldız **82**, İstanbul Hilal **79**, Karaköy Kartalları **77**, Trabzon Fırtına **75**; diğerleri **59–67**. |

Yedek ve gelişim kadroları ilk 11 seviyesine topluca çıkarılmaz; küçük kulüpte istisnai bir yıldız bulunabilir. İngiltere ve İtalya'daki üst grup birbirine yakınken Fransa'nın lideriyle takipçileri arasındaki fark daha büyüktür. Orta sıra takımlarının farklı seviyeleri korunur.

Pas yeteneği baskı ve sprint sırasında gerçek vuruş yönünün hassasiyetini belirler; baskısız normal pas seçilen yönü korur. Bitiricilik, baskılı, uzak ve yüksek güçlü şutların isabetini etkiler; şut önizlemesi aynı hesabı kullanır. Kontrolü iyi oyuncu daha kısa ilk dokunuşla topu yakında tutar; sprintte zayıf kontrol topu daha çok açar. Savunma yeteneği kaçırılan müdahalenin hareket toparlanmasını etkiler; yetenekli oyuncu da erişemediği topu kazanamaz. Hız, ivme, dayanıklılık ve kaleci özellikleri mevcut fizik hesaplarına katılmaya devam eder. Aynı kurallar iki takımda, kariyerde ve hızlı maçta uygulanır.

Eski kariyerin özellik ve potansiyelleri ilk yüklemede bir kez dönüştürülür. Önceden genel güç indirimini almış kayıtlar tekrar düşürülmeden lig dengesine geçirilir; mevcut antrenman kazanımları korunur. İkinci yetenek sürümündeki kayıtlar üçüncü sürüme geçerken yalnızca yeni bölgesel ayar farkını alır; değişmeyen liglerde eski bonus tekrar uygulanmaz, başlangıç tavanını antrenmanla aşmış oyuncu düşürülmez. Ayar oyuncunun ilk kimliğine bağlıdır: transferi, forma numarası, sözleşmesi, maaşı ve sezon kaydı korunur; yeniden yükleme veya transfer özellikleri tekrar üretmez. Yeni altyapı oyuncusu sırf büyük kulüpte doğduğu için hazır yıldız olmaz. Yeni kariyer maaşları yeni güçlere göre hesaplanır. `tests/player_talent_check.gd` dağılımı, kayıt dönüşümünü, altyapıyı ve gelişim sınırını; `tests/player_ability_check.gd` fiziksel pas sapmasını, ilk kontrolü, şutu, müdahale toparlanmasını ve gerçek transfer → kayıt → maç akışını sınar. `tests/league_strength_check.gd` lig hiyerarşisini, kadro derinliğini ve eski yetenek sürümlerinden geçişi; `tests/league_strength_flow_check.gd` güncellenen altı ligin hızlı maç kartlarını ve sahaya çıkan oyuncuların aynı özellikleri kullanmasını doğrular.

Bu değişikliğin regresyon kontrollerinde pas, nişan, top sürme, takım kimliği, kaleci dengesi, maç dinamikleri, gelişmiş hareketler, yağmur ve iki sezonluk kariyer testleri geçti. `reliable_reception_check.gd` içindeki 26 kontrolün 24'ü geçti; sert şutun geriye sekmesini bekleyen iki kontrol eski özellikler/ilk kontrol/top sürme hesabıyla yapılan karşılaştırmada da başarısız oldu. Bu mevcut fark, yeni yetenek dengelemesinden kaynaklanmıyor.


### Bağımsız SEFC ligleri

SEFC Premier Lig ve SEFC Birinci Lig, Türkiye Süper Ligi'nden ayrı, farklı ülkelerden oyuncuların buluştuğu kurgusal bir lig sistemidir. 36 kulübün kendine ait kurgusal şehirleri vardır: Coastal Rovers / Tidemouth, Atlas FC / Asteria, Ironvale FC, Solaris United, Port Azure Athletic, Kasumi City ve diğerleri. Başlangıçtaki 864 SEFC oyuncusu 13 ülkeye dağılır; ad ve soyadlar milliyetle uyumlu üretilir. Hızlı maç ve kariyer aynı oyuncu adlarını kullanır. Yeni SEFC altyapı oyuncuları da karma havuzdan gelir; belirli ülkeye gönderilen gözlemcinin raporu seçilen ülkeye bağlı kalır.

Kayıt yüklenirken eski SEFC kulüpleri ve başlangıç oyuncuları bir kez yeni kimliklerine geçirilir. Oyuncunun değişmez kimliği kullanılır: başka lige transfer olmuş SEFC oyuncusu güncellenir, SEFC'ye transfer olmuş Türkiye ligi veya diğer lig oyuncusunun adı ve milliyeti korunur. Önceden üretilmiş altyapı ve gözlemci oyuncularının kimlikleri korunur. Güçler, sözleşmeler, kadrolar, bütçeler, formalar, taktikler, yükselme/düşme, fikstürler ve sonuçlar değişmez. Türkiye Süper Ligi'nin 18 kulübü ve yerli/yabancı kadro yapısı aynıdır. Stadyum ve yayın metinleri SEFC Arena adını kullanır; iki SEFC liginin ortak kupası SEFC Lig Kupası'dır.

Doğrulama: `tests/sefc_identity_check.gd` yeni dünya, karma kadro, hızlı maç tutarlılığı, altyapı, iki yönde transfer, eski kayıt dönüşümü ve ülke filtrelerini sınar. `--visual` ile kariyer, hızlı maç ve kadro ekranlarını `/tmp/sefc-identity-*.png` olarak kaydeder.

**Canlı hücum komutları ve iptal:** Top sendeyken yönündeki arkadaşını **N / LB + R3** ile ileri koşuya gönder, **I / LB + L3** ile ayağına çağır. Seçim baktığın yöndeki uygun saha oyuncusuna yapılır; kaleci seçilmez. Koşular verkaçtan bağımsızdır, ofsayt çizgisini gözetir ve top kaybında biter. Ara pasın serbest nişanı değişmez.

**Şut / pas aldatması:** Şut, orta veya ara pası hazırlarken **S / kısa pas tuşuna** bas ve çıkış yönünü seç. Oyuncu vuruşu gösterip gerçek ayak temasıyla topu yana çeker; savunmacıya yapay sersemleme veya hız cezası verilmez. Temas bekleyen normal pas sırasında **Z** de pas aldatmasına geçer. Hazırlıktan tamamen vazgeçmek için **B / LB + RB** kullan. Bu komut bekleyen tek pası, gelişine vuruşu, orta/şut hazırlığını ve duran top koşusunu iptal eder; top çıktıktan sonra topun hareketini değiştirmez. Önceden basılı tuttuğun vuruş tuşunu bırakmak iptal edilen komutu yeniden başlatmaz. PlayStation'da LB/RB yerine L1/R1 kullanılır.

**Duran top organizasyonları:** Korner veya frikikte **1 / 2 / 3 / 4**, kontrolcüde **LB + D-pad yukarı / sağ / aşağı / sol** ile ön direk, arka direk, ceza yayı veya kısa pas seç. Arkadaşlar fiziksel olarak yerleşir ve vuruş koşusunda hareketlenir. Yön ve güç seçime göre otomatik değişmez. **B / LB + RB** top çıkmadan vuruş koşusundan da vazgeçer.

**Tekrarlanan hücum ve ikinci toplar:** Rakip tamamlanan kanat girişlerini zamanla öğrenir; uygun zorlukta bir orta saha oyuncusunu o kanadın çıkış yoluna kaydırır. Eski bölgesi boşalır, oyuncu özellikleri değişmez ve alışkanlık hafızası zamanla azalır. Dönen toplarda bir oyuncu inişi karşılar, biri tamamlama/kale koruma alanını, bir diğeri sonraki seken topun alanını alır. Küçük tahmin değişiklikleri ilk koşucuyu sürekli değiştirmez; gerçek sendeleme ve yorgunluk korunur.

### Hareket akışı, kadraj ve temas sunumu

Koşarken pas ve ayakla ilk kontrolde bacaklar canlı koşu adımına erken dönerken gövde ve kollar hareketi tamamlar. Topa gerçek temas ve destek ayağı çözümü önceliklidir; kontrol girdisi, oyuncu hızı ve pasın fizik hesabı animasyonu beklemez. Küçük yön değişiminde adım kısalır; sprintten duruşta ikinci destek basışı gelir, yeni yön komutu toparlanmayı hemen keser.

Maç kamerası uzun pasın gerçek uçuşundan kısa bir ileri tahmin alır; kontrada önde koşan arkadaşlara alan açar. Bakış yumuşak geçer, ek genişleme en fazla %6 / 2,5 derecedir. Taktik kamera ve duran top kadrajları korunur; duraklatma kamerayı sabitler.

Son pozdaki kramponun yerden ayrılıp yeniden basması aynı karede ayak izi ve yakın oyuncularda hafif bir çim sesi üretir. Kuru ve ıslak zemin ayrı duyulur. Yumuşak pas, sert top teması, şut ve eldiven için farklı sesler ve küçük yerel çeşitlilikler vardır; bu çeşitlilik maçın rastgelelik hesabını tüketmez. Ayak sesleri top temaslarının kanallarını kesmez. Forma kumaşına hafif kenar ışığı ve gölgede renk desteği eklenmiştir.

`tests/action_flow_check.gd` 30/60/120 Hz'de pas–koşu ve ilk kontrol geçişlerini, iki basışlı duruşu, ses/iz eşzamanlamasını, duraklatma/sessizliği ve iki hücum yönünde uzun pas kadrajını sınar.


### Menüler, erişilebilirlik ve maç raporu

Ana menü, hızlı maç, kariyer, ayarlar ve maç raporu ortak koyu zemin, açık yazı, açık yeşil işlem rengi ve altın ödül rengi kullanır. Seçim çerçevesi kısa bir geçişle odaklanan düğüme taşınır; gezinme, onay ve geçersiz tuş ataması farklı ses verir. **Ayarlar → Ses → Menü geri bildirim sesleri** ile kapatılır ve genel sessizlik ayarına uyar.

Kariyer merkezinde sıradaki maç ve açık, süresi dolmamış teklifler öne çıkar. Varsayılan işlem takvimi ilerletmek veya maça çıkmaktır. Kulüp rengi, arma, forma ve kaptanın 3D görüntüsü kullanılır; ilk maçına çıkmamış yeni transfer varsa vitrinde o görünür. Teklif sayacı doğrudan kulüp ekranını açar.

**Kadro / Transfer → Kart görünümü / Liste görünümü** aynı oyuncuları gösterir. Genel güç, hız, kondisyon, form ve bedelle sıralanır; liste sakatlık, ceza, yorgunluk ve ilk 11 durumunu yazıyla belirtir. Bölüm değiştirip geri dönmek filtreyi, seçilen oyuncuyu, sayfayı ve görünümü korur; yeniden sıralama seçili oyuncuyu kaybetmez. Karşılaştırmada öne çıkan olumlu ve olumsuz farklar gerçek özelliklerden hesaplanır.

Maçta gol ve kart sunumu, oyuncu değişikliği ve ilk maç tanıtımından önce gelir. Kesilen tanıtımların süresi durur, sonra kaldığı yerden devam eder. Normal canlı mesajlar sınırlı bir sırada bekler. **Ayarlar → Erişilebilirlik** altında skor tabelası, oyuncu kartı ve mini harita ayrı boyutlandırılır. **Arayüz hareketlerini azalt** seçeneği menü geçişlerini, odak kaymasını, kart yakınlaşmasını ve otomatik antrenman gösterimlerini durdurur; antrenmanın adımları elle ilerletilebilir.

**Ayarlar → Görüntü** seçilen kamerayı bağımsız bir önizlemede gösterir; maç kamerasını hareket ettirmez. **Tuş atama** ekranındaki kontrolcü çizimi yeni atamayı ve seçili tuşu gösterir. Ayar bölümüne geri dönünce odak ve kaydırma konumu korunur. Antrenmanın 12 çalışmasının her biri büyük saha çizimi, hareketli top yolu ve üç kısa açıklamayla anlatılır; gösterim duraklatılabilir.

Son düdükten sonra **Maçın özeti / Oyuncu notları / İstatistikler** sekmeleri açılır. Gol, kart ve tamamlanan değişiklikler zaman sırasıyla gösterilir; gol, şut, kurtarış, topa müdahale ve kartlardan hesaplanan maç notuna göre maçın oyuncusu seçilir. Notlar bir oyun içi değerlendirmedir; kullanılmayan yedeklere not verilmez. Değişiklikte iki oyuncunun kimliği ve dakikası ayrı tutulur. Kaleciden sekip giren şut, şutu atan oyuncuya yazılır. Kariyerin mevcut sonuç işlemi tamamlandıktan sonra puan, sıra, kasa değişimi ve sakat/cezalı oyuncu sayısı gösterilir; ekran ödülleri yeniden uygulamaz. Kupa töreni aynı rapora döner. Ekran raporu, isteğe bağlı yerel veri kaydı kapalıyken de çalışır.

`tests/ui_upgrade_check.gd` eski ayar uyumluluğunu, kayıt dönüşünü, canlı kamera izolasyonunu, tuş atamasını, odak/kaydırmayı, liste sıralamasını, azaltılmış hareketi, bildirim önceliğini, oyuncu kimliklerini ve tek seferlik kariyer sonuçlarını sınar. Grafik çalıştırmada `-- --visual` ile ekran görüntüleri `/tmp/sefc-ui-*.png` altına alınır. Mevcut ana menü, kontrolcü, kariyer, kamera, görüntü, ekran oranı, antrenman ve sunum akış testleriyle birlikte doğrulanır.
