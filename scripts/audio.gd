...
func react(kind: String,_team: int,_location: Vector3) -> void:
	...
	var priority: int={"goal":4,"save":3,"shot":2,"miss":1,"tackle":1}.get(kind,0)
	...
	if not was_playing:
		cheering.volume_db=-80
		# The supplied clip has about 0.8 seconds of leading silence.
		cheering.play(0.8)
	else:
		# Keep the applause continuous when a shot becomes a save or a goal.
