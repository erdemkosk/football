...
func _process(delta: float) -> void:
	...
	if not is_equal_approx(before,emphasis) or kind=="player": queue_redraw()
...
func _draw() -> void:
	...
	var base:=Color("192b3d").lerp(Color("304557"),emphasis*.8)
	...
	if kind=="player":
		...
		draw_rect(Rect2(1,119,size.x-2,size.y-120),Color("122332"))
