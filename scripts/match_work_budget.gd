...
func decision_delta(index: int,delta: float,nearest: Array) -> float:
	...
	var urgent: bool=not enabled or game.training or p.keeper or game.is_user_player(index) or distance<18.0*18.0 or not p.can_defer_running_pose()
	...
	if urgent:
		var elapsed: float=entries.get(index,{}).get("elapsed",0.0)
		entries.erase(index)
