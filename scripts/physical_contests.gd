	...
	for a in range(1,11):
		...
		for b in range(12,22):
			...
			if distance<.92:
				...
				p.velocity+=impulse*(76.0/p.weight_kg)/(p.Attributes.multiplier(p.attributes.get("strength",72),.22)*(1.12 if p.Attributes.has_style(p,"bruiser") else 1.0))
				q.velocity-=impulse*(76.0/q.weight_kg)/(q.Attributes.multiplier(q.attributes.get("strength",72),.22)*(1.12 if q.Attributes.has_style(q,"bruiser") else 1.0))
			pairs.append([a,b]); paired.append(a); paired.append(b)
			break
	for i in range(game.players.size()):
		if i not in paired: game.players[i].contest_weight=move_toward(game.players[i].contest_weight,0,delta*4)
