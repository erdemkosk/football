...
func update(delta: float) -> void:
	...
	for a in range(1,11):
		...
		for b in range(12,22):
			...
			if not air and (p.velocity.length()<1 or q.velocity.length()<1 or p.facing.dot(q.facing)<.3 or absf(gap.normalized().dot(p.facing))>.65): continue
			...
			if distance<.92:
				...
				p.velocity+=impulse*(76.0/p.weight_kg)
				q.velocity-=impulse*(76.0/q.weight_kg)
