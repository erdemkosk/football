...
func act_and_contact(index: int) -> bool:
	if not game.ai_attack.act(index): return false
	# Decisions request a kick; the ordinary boot animation commits its physics.
...
func run() -> void:
	...
	check(kind()=="shot" and act_and_contact(20) and game.shots[1]==1 and game.ball.pending_kick,"A clear scoring position produces a physical shot")
	...
	check(kind()=="one_two" and act_and_contact(20),"Nearby pressure with a safe wall player triggers a one-two")
	...
	possession(18)
	check(kind(18)=="return_pass" and act_and_contact(18) and game.passes[1]==2 and not game.support.runs.has(20),"The wall player returns the ball and ends the original run")
	...
	check(act_and_contact(20) and game.last_kicker==20 and game.ai_receivers[1]==18,"The cross is kicked by the winger with no ball teleport")
	setup(Vector3(23,0,44)); player(18,Vector3(4,0,38))
	check(kind()=="driven_cross" and act_and_contact(20) and game.ball.kick_velocity.y<.2 and game.ball.kick_velocity.length()>27,"A clear byline cutback becomes a hard ground cross")
	setup(Vector3(20,0,4)); player(18,Vector3(-24,0,6)); player(4,Vector3(24,0,4))
	check(kind()=="switch" and act_and_contact(20) and game.ball.kick_velocity.x< -10 and game.ball.kick_velocity.y>6,"Pressure on one wing prompts a lofted switch toward the free opposite wing")
	...
	check(kind()=="clearance" and act_and_contact(20) and game.ball.kick_velocity.z>8,"A trapped defender with no safe outlet clears toward the flank")
	setup(); player(9,Vector3(0,0,-30)); possession(9)
	check(not act_and_contact(9) and not game.ball.pending_kick and game.passes[0]==0 and game.shots[0]==0,"The user team never passes, shoots, or performs possession skills without permission")
	setup(Vector3(0,0,33)); game.ball.position.x=5
	check(not act_and_contact(20) and not game.ball.pending_kick,"AI decisions cannot remotely kick a ball outside foot reach")
	...
	check(act_and_contact(20) and game.ball.kick_velocity.z<0,"AI attacking decisions reverse with second-half ends")
