...
func run() -> void:
	...
	setup(); button(JOY_BUTTON_B)
	check(game.passes[0]==0 and not game.ball.pending_kick,"Holding cross starts preparation without kicking")
	game.update_control(DT)
	check(game.players[9].shot_preparation>0,"The held cross has a visible kicking preparation")
	game.controller.combos.update(1.4)
	check(game.passes[0]==0 and game.controller.combos.cross_power==1,"Full power still waits for release")
	button(JOY_BUTTON_B,false); contact()
	check(game.passes[0]==1 and game.ball.kick_velocity.y>3,"Release commits one airborne cross")
	setup(); button(JOY_BUTTON_RIGHT_SHOULDER); button(JOY_BUTTON_B); game.controller.combos.update(.55); button(JOY_BUTTON_B,false); button(JOY_BUTTON_RIGHT_SHOULDER,false)
	contact()
	var velocity: Vector3=game.ball.kick_velocity
	check(game.passes[0]==1 and velocity.y<0.2 and velocity.length()>=27,"Sprint plus cross produces one hard ground cross")
	check(game.controlled==9 and game.shots[0]==0,"Driven cross keeps the passer selected and counts as a pass")
	tap(JOY_BUTTON_B)
	check(game.passes[0]==1 and game.players[6].pose!="slide","A rapid repeat cannot cause a second kick or an accidental receiver slide")
	setup(); button(JOY_BUTTON_B); button(JOY_BUTTON_B)
	game.controller.combos.update(0.24); button(JOY_BUTTON_B,false); contact()
	check(game.passes[0]==1 and game.ball.kick_velocity.y>3,"Repeated held button events do not change the cross type")
	setup(); button(JOY_BUTTON_B); game.ball.position+=Vector3(6,0,0); game.controller.combos.update(0.1); button(JOY_BUTTON_B,false)
	check(game.passes[0]==0 and game.players[9].pose!="slide","Losing the ball during the hold cancels without a remote kick or slide")
	setup(); button(JOY_BUTTON_B); tap(JOY_BUTTON_START); tap(JOY_BUTTON_A); game.controller.combos.update(0.5); button(JOY_BUTTON_B,false)
	check(game.state=="playing" and game.passes[0]==0,"Pause and resume discard a pending cross")
	setup(); button(JOY_BUTTON_B); game.controller.connection_changed(0,false); game.controller.combos.update(0.5)
	check(game.state=="paused" and game.passes[0]==0,"Disconnect discards the pending cross")
	setup(); button(JOY_BUTTON_B); button(JOY_BUTTON_X,true,1); game.controller.combos.update(0.4); button(JOY_BUTTON_B,false); contact()
	...
	button(JOY_BUTTON_RIGHT_SHOULDER); button(JOY_BUTTON_B); game.controller.combos.update(.55); button(JOY_BUTTON_B,false); button(JOY_BUTTON_RIGHT_SHOULDER,false)
	contact()
