...
func roll(mode: int,speed: float,origin: Vector3=Vector3(15,0.23,-25),direction: Vector3=Vector3.BACK,rate: int=120) -> Dictionary:
	...
	var result := {"distance":game.flat_distance(start,stop),"time":elapsed,"initial":initial,"after_second":halfway,"monotonic":monotonic,"smooth":smooth,"stopped":stopped,"drift":drift}
...
func run() -> void:
	...
	var dry_short: Dictionary=await roll(0,4,Vector3(0,0.23,47),Vector3.RIGHT)
	var muddy: Dictionary=await roll(2,4,Vector3(0,0.23,47),Vector3.RIGHT)
	...
	var rate30: Dictionary=await roll(0,8,Vector3(15,0.23,-25),Vector3.BACK,30)
	var rate60: Dictionary=await roll(0,8,Vector3(15,0.23,-25),Vector3.BACK,60)
	...
	game.weather.select(2,true); game.ball.place(Vector3(-10,0.23,0),Vector3(10,0,0)); await frames(3)
	...
	var origin := Vector3(-25,1.6,15); var target := Vector3(-12,0.23,15); var flight := 1.6
	...
	for n in range(720):
		await physics_frame
		if not landed and n>30 and game.ball.position.y<0.27:
		...
		if landed and game.ball.position.y<0.30 and absf(game.ball.linear_velocity.y)<1.2: ground_time+=1.0/120
	...
	game.weather.select(0,true)
	var dry_route=game.Passing.plan(Vector3(-3,0.23,47),Vector3(3,0,47),Vector3.ZERO,false,game.weather)
	game.weather.select(2,true)
	var muddy_route=game.Passing.plan(Vector3(-3,0.23,47),Vector3(3,0,47),Vector3.ZERO,false,game.weather)
	...
	game.ball.place(Vector3(15,0.23,-10),Vector3(0,0,8)); await frames(3)
