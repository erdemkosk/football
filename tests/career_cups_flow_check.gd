...
func run() -> void:
	...
	tap(JOY_BUTTON_DPAD_RIGHT)
	check(ui.nation_filter=="TR" and not ui.list_ids.is_empty() and ui.list_ids.all(func(pid): return c.player(pid).nationality=="TR"),"The independent SEFC market filters by country without treating Turkish players as its domestic baseline")
