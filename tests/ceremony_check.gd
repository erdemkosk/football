...
func run() -> void:
	...
	check(game.state=="playing" and "presentation" in seen and "formation" in seen,"The complete walkout, salute and formation sequence reaches kickoff")
	...
	for stop in ["walkout","presentation","formation"]:
		...
		key(KEY_SPACE)
		check(game.state=="playing" and game.ceremony.phase=="" and game.ball.active,"One Space press skips safely from "+stop)
	game.start_match()
	game.return_menu()
	check(game.state=="menu" and game.ceremony.phase=="" and not game.ceremony.officials[0].visible,"Returning to the menu cleans up all ceremony actors")
