...
func run() -> void:
	...
	var far_end: float=crowd.follow_weight(fposmod(atan2(44.0/60.0,-28.0/45.0)/TAU,1.0))
	check(crowd.follow>0.4 and near_end>far_end+0.2,"The stand nearest the ball leans in while the far stand stays down")
	crowd.reset()
	advance(1,Vector3(0,0,-44),Vector3(0,0,-22),true)
