...
func run() -> void:
	...
	check(game.pass_preview.receiver==-1 and game.pass_preview.lob,"Short press previews an aerial pass along the aimed heading")
	var short_target: Vector3=game.pass_preview.target
	charge(0.52)
	check(game.pass_preview.receiver==-1 and game.pass_preview.velocity.y>9 and game.pass_preview.target.distance_to(game.ball.position)>short_target.distance_to(game.ball.position)+8,"Holding Y sends the lofted pass farther along the same heading")
	await capture()
	...
	check(game.passes[0]==1 and game.controlled==9 and intended.is_equal_approx(route.velocity) and game.ball.kick_velocity.is_equal_approx(game.strike_quality.last.velocity),"Releasing Y kicks the previewed physical pass and leaves the passer selected")
