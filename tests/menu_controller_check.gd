...
func run() -> void:
	...
	check(focus()==game.hud.nav_buttons[1],"Left analog moves one row immediately")
	game.controller.menus.update(0.2)
	check(focus()==game.hud.nav_buttons[1],"Held stick waits before repeating")
	game.controller.menus.update(0.17)
	check(focus()==game.hud.nav_buttons[2],"Held analog repeats at a controlled rate")
	axis(JOY_AXIS_LEFT_Y,0)
	game.controller.menus.update(1)
	check(focus()==game.hud.nav_buttons[2],"Centering the stick stops repetition")
