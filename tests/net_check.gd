...
func run() -> void:
	...
	check(strong.spread>0.22,"The surrounding net has a pronounced wave outside the ball footprint")
	check(strong.recoil>0.15,"The net visibly vibrates back past rest after catching the ball")
	check(strong.wide_spread>0.11,"Impact travels more than 1.6 metres across the surrounding net")
	check(strong.aftershock>0.15,"A strong shot still visibly shakes the net after 0.8 seconds")
