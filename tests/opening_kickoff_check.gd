...
func run() -> void:
	...
	for mode in ["keyboard","xbox","playstation"]:
		game.start_match()
		game.controller.device=0; game.controller.reset_bindings(); game.controller.family=mode if mode!="keyboard" else "xbox"
		await physics_frame; await physics_frame
		if mode=="keyboard": key(KEY_SPACE); key(KEY_SPACE,false)
		...
		if mode=="keyboard": key(KEY_S); key(KEY_S,false)
		else: pad(true); pad(false)
		await tick(85)
		...
		if "--visual" in OS.get_cmdline_user_args() and mode=="keyboard":
