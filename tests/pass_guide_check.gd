...
func run() -> void:
	...
	for weather in [0,2]:
		for lob in [false,true]:
			...
			var origin := Vector3(12,0.23,-12)
			var target := Vector3(-6,0.23,-30)
			...
			check(Array(route.points).all(func(p): return p.y>=0.219),"Pass and cross guides stay above the pitch (weather %d, lob %s)" % [weather,lob])
			...
			for frame in range(720):
				...
				if lob:
					...
					if descended and point.y<0.25: actual=point; break
	...
	setup()
	var weak := {"target":Vector3(20,0.23,-46),"velocity":Vector3(0,0.32,-10),"flight":6.0,"lob":false,"receiver":-1}
	...
	game.ball.position=game.restart_point+Vector3.UP*0.23; game.ball.pending_reset=false
