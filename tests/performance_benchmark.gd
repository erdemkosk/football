extends SceneTree
## Wall-clock benchmark. Do not use --fixed-fps for graphical measurements.
var game
var label := "before"
var results: Array=[]
const DT := 1.0/120
...
func sample(scene: String,live: bool,period: int,rain: bool=false,wide: bool=false) -> void:
	...
	var row := {"scene":scene,"fps":count/duration,"p50_ms":samples[count/2],"p95_ms":samples[int(count*.95)],"p99_ms":samples[int(count*.99)],"render_cpu_ms":cpu_ms/count,"gpu_ms":gpu_ms/count,"physics_ms":physics_ms/count,"physics_hz":(Engine.get_physics_frames()-ticks)/duration,"draws":draws/count,"frames":count}
	results.append(row); print(JSON.stringify(row))
...
func run() -> void:
	...
	if "--planning" in OS.get_cmdline_user_args():
		compare_planning()
	elif "--cpu" in OS.get_cmdline_user_args():
		cpu("support",func(): game.support.update(DT))
		cpu("defensive-plan",func(): game.team_tactics.update(.5))
		cpu("attack-decision",func(): game.ai_attack.decide(20),40)
		cpu("all-ai",func(): game.update_ai(DT))
		cpu("animate-22",func():
			for p in game.players: p.animate(DT))
		cpu("crowd",func(): game.stadium.crowd.update(DT,game.ball.position,Vector3.ZERO,1,true,false))
		cpu("sidelines",func(): game.stadium.sidelines.update(DT,game.ball.position,Vector3.ZERO,1,true,"",Vector3.ZERO))
		cpu("weather",func(): game.weather.update(DT))
	else:
		print("BENCHMARK ",label," size=",root.size," scale=",root.scaling_3d_scale," renderer=",RenderingServer.get_current_rendering_method()," physics=",Engine.physics_ticks_per_second)
		await sample("day-static",false,0)
		await sample("day-match",true,0)
		await sample("night-match",true,1)
		await sample("rain-match",true,1,true)
		await sample("stadium",false,0,false,true)
	var file := FileAccess.open("res://tests/performance-"+label+".json",FileAccess.WRITE)
