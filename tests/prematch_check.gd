...
func run() -> void:
	...
	check(game.state=="set_piece" and game.restart_type=="SANTRA" and game.team_name(1)=="GÜNEŞ FK","Selected opponent survives the ceremony and waits at kickoff")
	...
	game.frontend.confirm()
	check(game.state in ["restart","set_piece"] and game.restart_type=="SANTRA" and not game.ball.freeze,"Closing tactics applies queued substitutions before the pending opening kickoff")
	game.return_menu()
	...
	game.frontend.cycle_league(1,9)
	check(game.clubs.league_name(1)=="Türkiye Süper Ligi" and game.team_name(0)!=game.team_name(1),"Quick match can pick a league and see that league's clubs")
	...
	game.frontend.cycle_league(0,1)
	check(game.frontend.previews.all(func(p): return not p.performing and not p.chosen),"Preview players stand idle until a side is picked")
	game.frontend.pick_current()
	check(game.frontend.picked[0] and game.frontend.previews[0].chosen and game.frontend.stage=="teams","Picking a team plays the selected pose")
	game.frontend._process(1.0)
	check(game.frontend.pick_step==1 and not game.frontend.picked[1],"After the home pose, focus moves to the opponent")
	game.frontend.pick_current()
	game.frontend._process(1.0)
	check(game.frontend.pick_step==2 and game.frontend.picked[1] and game.frontend.stage=="teams","After both teams are picked, the next button is focused")
	game.frontend.pick_current()
	check(game.frontend.stage=="tactics","A third confirm opens tactics")
	game.frontend.back()
	check(game.frontend.stage=="teams" and game.frontend.visible,"Back from tactics returns to team selection")
	game.frontend.back()
	check(game.frontend.picked[0] and not game.frontend.picked[1],"B undoes the opponent selection")
	game.frontend.back()
	check(not game.frontend.picked[0] and game.frontend.pick_step==0,"B undoes the home selection")
	game.frontend.back()
