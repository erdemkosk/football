...
func run() -> void:
	...
	for period in [0,2,1]:
		...
		check(not game.stadium.env.glow_enabled,"Night does not bloom floodlights or the pitch: "+str(period))
	...
	check(not game.stadium.env.glow_enabled,"Rainy night keeps bloom off")
	point_camera(Vector3(30,1.4,-24),Vector3(-13,1,12),62)
	await capture("wet-floodlight")
	game.weather.select(0,true); rig.select(0)
	check(game.stadium.grass.get_shader_parameter("flood_glint")==0.0 and not game.stadium.env.fog_enabled,"Night and day keep the pitch free of fog and floodlight glare")
