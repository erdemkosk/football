...
func run() -> void:
	...
	check(game.players.filter(func(p): return p.visible).size()==2 and game.controlled==9,"Free practice retains only the controlled attacker and goalkeeper")
	await tick(5)
	key(KEY_T); key(KEY_T,false)
	...
	check(game.players[8].visible and game.players.filter(func(p): return p.visible).size()==3 and game.controlled==9,"A teammate joins on the wing while control stays on the finisher")
	await tick(20)
	...
	key(KEY_R); key(KEY_R,false); await tick(60)
	check(game.state=="set_piece" and game.restart_point!=old_point and game.training_drills.attempt==2,"Retry from a set piece offers a different shooting angle")
	...
	check(game.state=="set_piece" and game.practice_goals==1 and game.training_drills.mode=="free_kick","Skipping a training goal returns to the wall, without an accidental live-play reset")
	game.begin_restart("TAÇ",1,Vector3(32,0,-30)); await tick(400)
	check(game.state=="set_piece" and game.training_drills.mode=="free_kick","An out-of-play ball automatically sets up another free kick")
	...
	check(game.training_drills.mode=="free_kick" and game.state=="set_piece","Mouse selection and the start button open the selected exercise")
	key(KEY_T); key(KEY_T,false); game.training_menu.back_button.grab_focus(); key(KEY_ENTER); key(KEY_ENTER,false)
	check(not game.training_menu.visible and game.state=="set_piece" and not game.ball.freeze,"Enter on Back returns to the current free kick without restarting it")
	game.return_menu()
